...
const CampaignCard: React.FC<CampaignCardProps> = ({ campaign, onSelect }) => {
  ...
  const getCategoryImage = (category: string) => {
    const categoryImages: Record<string, string> = {
      ...
      education: "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80",
  ...
  const getCategoryColor = (category: string) => {
    const categoryColors: Record<string, { bg: string, text: string }> = {
      ...
      education: { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-300' }, // Using blue for education
      ...
      rent: { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-300' }, // Using blue for rent
      fund: { bg: 'bg-teal-100 dark:bg-teal-900/30', text: 'text-teal-700 dark:text-teal-300' },
    };
    
    return categoryColors[category.toLowerCase()] || { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-700 dark:text-blue-300' }; // Default to blue
  };

  // Function to generate a default description based on each category
  ...
  // Use the campaign description if available, if not generate a default one
  const description = campaign.description || getDefaultDescription(campaign.category, campaign.title);

  return (
    <div 
      className="campaign-card bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden cursor-pointer hover:shadow-lg transition-shadow duration-300 border border-blue-100 dark:border-blue-900/50"
    ...
    >
      ...
      <div className="h-48 relative overflow-hidden">
        <img 
          ...
          onError={(e) => {
            //blue-themed image incase the original fails
            e.currentTarget.src = 'https://images.unsplash.com/photo-1552664730-d307ca884978?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80';
          }}
        />
        ...
        <div className="absolute bottom-4 right-4 bg-white/90 dark:bg-gray-800/90 text-gray-600 dark:text-gray-300 text-xs font-medium px-2 py-1 rounded">
          <i className="far fa-clock mr-1"></i> {timeLeft}
        </div>
      </div>
      
      {/* Campaign Content */}
      <div className="p-5">
        <h3 className="font-bold text-lg mb-2 line-clamp-1 text-gray-800 dark:text-white">{campaign.title}</h3>
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm mb-1">
            <span className="text-gray-600 dark:text-gray-300">Raised: {campaign.raised} ETH</span>
            <span className="text-gray-600 dark:text-gray-300">Goal: {campaign.goal} ETH</span>
          </div>
        ...
        <div className="flex justify-between text-sm mb-4">
          <span className="text-gray-500 dark:text-gray-400">
            <i className="fas fa-users mr-1"></i> {campaign.backers} backers
          </span>
          <span className="text-gray-500 dark:text-gray-400">
            <i className="fas fa-map-marker-alt mr-1"></i> {campaign.location}
          </span>
        </div>  
        ...
        } text-white py-2 rounded-lg transition-colors duration-200 font-medium`}>
